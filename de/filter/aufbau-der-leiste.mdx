...
*   **Globaler Pfad-Filter**: Über das Pfad-Feld setzen Sie einen Pfad-Filter, der **report-übergreifend** wirkt. Anders als ein einzelner Widget-Filter bleibt diese Einschränkung über alle Berichte hinweg aktiv – so analysieren Sie z.B. einen bestimmten Seitenbereich durchgängig in Dashboard, Pfad-Analyse und UTM-Berichten.

*   **"Alle"-Textsuche**: Mit dem Feld `Alle` durchsuchen Sie **alle Felder gleichzeitig**. Statt sich für eine einzelne Dimension (URL, Referrer, Land …) zu entscheiden, geben Sie einen Suchbegriff ein, der über sämtliche Felder hinweg geprüft wird.

*   **Schließen-Button (X)**: Mit diesem Button können Sie die Filterleiste jederzeit ausblenden.

<Note>
    Bei bedingungsbasierten Filtern steht Ihnen zusätzlich zu Operatoren wie *ist*, *ist nicht* oder *enthält* nun auch der **REGEX-Operator** zur Verfügung. Damit filtern Sie über reguläre Ausdrücke – ideal für komplexe Muster, etwa mehrere URL-Varianten in einer einzigen Bedingung.
</Note>
